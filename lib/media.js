...
export const work = [
  ...
  // Was card 05, and is now unused: the card took the spiral-of-code picture
  // below when the seventh service was dropped. Kept because the file is in
  // the repo and the alt text with it.
  //
  // Replaced 1 August: the supplied jigsaw graphic arrived at 235x146
  // — an order of magnitude under everything else in the grid, so the 4:5 frame
  // upscaled it about six times and it read as a thumbnail. Swapped for a CRM
  // contact record shot at 2240x2800 and downsampled to 1120x1400, which is the
  // frame exactly, so nothing crops. It is also the one image in the grid that
  // shows the service rather than a metaphor for it.
  {
    id: 'crm-attribution',
    alt: 'A laptop screen showing a CRM contact record open on Add new contact',
    caption: 'Attribution',
  },
  // Card 05 now — it was card 07's until that service was dropped, and the id
  // is kept rather than renamed because the file in /public carries it.
  //
  // Arrived as a 1152x2048 PNG with a 101px grey letterbox band across
