...
export const site = {
  ...
  // Every enquiry on the site routes here — this one line, not a scattered
  // set of addresses. It feeds the contact form's recipient (see
  // app/api/contact/route.js, which reads process.env.CONTACT_TO and falls
  // back to this value), both mailto: links (Nav's mobile sheet and the
  // homepage contact section), and the JSON-LD contact record in the root
  // layout. Change it here and every one of those follows.
  ...
  //
  // Deliberately NOT `email`. This was written when the form's recipient was
  // still info@sharoon.ae, a mailbox with no MX records — mail sent there
  // bounced, and telling someone whose enquiry just failed to use a second
  // dead channel would have lost them twice. The email above now points at
  // info@shario.ae instead; whether that mailbox is live hasn't been
  // confirmed from this environment, so the fallback stays on the phone
  // line, which is answered today, until delivery there is verified.
  //
  // Once it is, change this one line back to `Email ${site.email}` and every
  // message follows.
...
//
// The anchors are rooted at "/" rather than written bare. A bare "#services"
...
// nothing at all. "/#services" goes home first and then scrolls.
//
// "System" was pulled from this list on request — the section itself (the
// diagram at id="system" on the homepage) is untouched and still reachable by
// scrolling or by any link that points at /#system directly; it simply no
// longer has a slot in the header. Nav is a flex row and the mobile sheet is
// a plain mapped list, so removing an entry here is the whole change — both
// close the gap on their own, nothing left to reclaim by hand.
export const nav = [
  { label: 'Services', href: '/#services' },
  { label: 'Results', href: '/#results' },
  // Points at the standalone /about page (app/(site)/about), not the
  // homepage's own #about panel — that section is untouched and still
  // exists, it's just no longer where this link goes.
